...
{
    LOG_INFO() << "void MainWindow::open(Mlt::Producer *producer)";
...
{
    LOG_INFO() << "void MainWindow::doAutosave() ... start";
    QMutexLocker locker(&m_autosaveMutex);
    if (m_autosaveFile) {
        bool success = false;
    ...
    LOG_INFO() << "void MainWindow::doAutosave() ... end";
}
...
{
    LOG_INFO() << "static void autosaveTask(MainWindow *p)  ... begin" ;
    LOG_DEBUG_TIME();
    p->doAutosave();
    LOG_INFO() << "static void autosaveTask(MainWindow *p)  ... end" ;
}

void MainWindow::onAutosaveTimeout()
{
    LOG_INFO() << "void MainWindow::onAutosaveTimeout()  ... begin" ;
    if (isWindowModified()) {
    ...
    LOG_INFO() << "void MainWindow::onAutosaveTimeout()  ... end" ;
}
...
{
    LOG_INFO() << "void MainWindow::openMultiple(const QStringList &paths)";
    if (paths.size() > 1) {
        QList<QUrl> urls;
...
void MainWindow::keyPressEvent(QKeyEvent *event)
{
    ...
    case Qt::Key_Left:
        if ((event->modifiers() & Qt::ControlModifier) && m_timelineDock->isVisible()) {
            if (m_timelineDock->selection().isEmpty()) {
                ...
                if (newIndex < 0)
                    break;
                m_timelineDock->setSelection(QList<QPoint>() << QPoint(newIndex,
                                                                       m_timelineDock->selection().first().y()));
    ...
    case Qt::Key_L:
        ...
        if (event->modifiers() & Qt::ControlModifier && isMultitrackValid())
#endif
            m_timelineDock->setTrackLock(m_timelineDock->currentTrack(),
    ...
    case Qt::Key_B:
        if (event->modifiers() & Qt::ControlModifier && event->modifiers() & Qt::AltModifier) {
            ...
            if (!isBottomVideo) {
                bool isComposite = m_timelineDock->model()->data(m_timelineDock->model()->index(trackIndex),
...
{
    LOG_INFO() << "bool MainWindow::saveXML(const QString &filename, bool withRelativePaths) begin";
    bool result;
    ...
    if (m_timelineDock->model()->rowCount() > 0) {
     ...
     LOG_INFO() << "bool MainWindow::saveXML(const QString &filename, bool withRelativePaths) end";
    return result;
}
...
void MainWindow::on_actionCopy_triggered()
{
    ...
    m_timelineDock->raise();
    if (!m_timelineDock->selection().isEmpty())
        m_timelineDock->copy(m_timelineDock->selection().first().y(),
                             m_timelineDock->selection().first().x());
}
